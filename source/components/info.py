...
class Info():
    ...
    def create_info_labels(self):
        ...
        if self.game_info[c.PLAYER_NAME] == c.PLAYER_MARIO:
            player = "MARIO"
        elif self.game_info[c.PLAYER_NAME] == c.PLAYER_LUIGI:
            player = "LUIGI"
        else:
        ...
        self.info_labels = [ self.mario_label, self.world_label, self.time_label, self.stage_label]
    ...
    def create_main_menu_labels(self):
        ...
        self.state_labels = [mario_game, luigi_game, top, 
                            *self.info_labels]
    ...
    def update_text(self, text, text2, reset=False):
        if reset and len(text) > len(str(text2)):
            text.remove(text[0])
        index = len(text) - 1
        for digit in reversed(str(text2)):
